...
`ifndef VCS
  // 注：以下为 generate 作用域的 elaboration 配置断言。VCS 解析器不支持在
  //   generate 作用域直接写 $fatal/$warning(报 "token is ';'")；Vivado 综合可。
  //   本项目 FPU 为 FP32-only 配置，该断言条件恒假(inert)，故用 `ifndef VCS 让
  //   VCS 跳过解析，Vivado 综合路径完全不受影响(VCS 宏仅 VCS 预定义)。
