// 显式include宏定义头(FS_TO_DS_BUS_WD等总线位宽宏)。此前本文件靠filelist里
// mycpu.h排在前面当"全局宏"，但Vivado 2023.1的read_verilog不保证.h的`define跨
// 文件可见(2024.2可容忍)，导致综合报undefined macro。mycpu.h有`ifndef guard，
// 与作为源文件读入不冲突；同目录其他文件(csr.v/id_stage.v等)本就是这样include的。
`include "mycpu.h"
...
// FPU wires (ports exist unconditionally on stage modules)
wire [`WS_TO_FPR_BUS_WD -1:0] ws_to_fpr_bus;

// FPU execution (exe_stage <-> fpu_top)
wire        fpu_req_valid;
wire        fpu_req_ready;
wire [ 4:0] fpu_req_dest;
wire [ 4:0] fpu_req_op;      // 阶段2: 4→5bit
wire [31:0] fpu_req_a;
wire [31:0] fpu_req_b;
wire [31:0] fpu_req_c;   // fmadd/fmsub 第三源 fa
wire [ 4:0] fpu_fcmp_cond;  // 阶段2: 3→5bit, 透传手册 cond 原值
wire        fpu_req_tag; // [AI FPU Refactor] NEW
wire        fpu_rsp_valid;
wire [31:0] fpu_rsp_result;
wire [ 4:0] fpu_rsp_dest;
wire        fpu_rsp_tag; // [AI FPU Refactor] NEW
wire        fpu_fcmp_out;
wire        ds_fpu_epoch; // [AI FPU Refactor] NEW

// FCC (wb_stage -> fpu_ctl)
wire        fcc_we;
wire [ 1:0] fcc_cond;
wire [ 4:0] fcc_fj;
wire        fcc_result;

// FCC read (fpu_ctl -> id_stage)
wire        fcc_val;
wire [ 2:0] fcc_idx;

// FPU 状态/标志 与 FCSR 软件读写 —— 必须在 id_stage 实例化之前声明,
// 否则在端口连接处先被当作隐式 1-bit 网络, 多位信号会被截断 (见 IPDW/PCWM 警告)。
wire        fpu_rsp_is_fcmp;
wire [ 4:0] fpu_rsp_flags;
wire        fpu_flags_we;
wire [ 4:0] fpu_flags;
wire [ 2:0] fcsr_rm;
wire        fcsr_we;       // movgr2fcsr 写使能 (id_stage -> fpu_ctl)
wire [31:0] fcsr_wdata;    // movgr2fcsr 写数据
wire [31:0] fcsr_rdata;    // movfcsr2gr 读数据 (fpu_ctl -> id_stage)
...
id_stage id_stage(
    ...
    .ws_to_fpr_bus        (ws_to_fpr_bus       ),
    //from fpu_top: standalone FPU write back
    .fpu_rsp_valid        (fpu_rsp_valid       ),
    .fpu_rsp_tag          (fpu_rsp_tag         ),
    .fpu_rsp_dest         (fpu_rsp_dest        ),
    .fpu_rsp_result       (fpu_rsp_result      ),
    .fpu_rsp_flags        (fpu_rsp_flags       ),
    .fpu_rsp_is_fcmp      (fpu_rsp_is_fcmp     ),
    //from fpu_ctl: FCC read
    .fcc_val              (fcc_val             ),
    .fcc_idx              (fcc_idx             ),
    .ds_fpu_epoch         (ds_fpu_epoch        ),
    //to fpu_ctl: FCC async write back
    .fcc_we_o             (fcc_we              ),
    .fcc_fj_o             (fcc_fj              ),
    .fcc_result_o         (fcc_result          ),
    .fpu_flags_we_o       (fpu_flags_we        ),
    .fpu_flags_o          (fpu_flags           ),
    //FCSR software read/write (movgr2fcsr / movfcsr2gr)
    .fcsr_rdata           (fcsr_rdata          ),
    .fcsr_we_o            (fcsr_we             ),
    .fcsr_wdata_o         (fcsr_wdata          )
    `ifdef DIFFTEST_EN
    ,
    .rf_to_diff           (regs                )
    `endif
...
`ifdef HAS_FPU
    ...
    .fpu_req_b            (fpu_req_b           ),
    .fpu_req_c            (fpu_req_c           ),
    .fpu_req_dest         (fpu_req_dest        ),
    .fpu_fcmp_cond        (fpu_fcmp_cond       ),
    .fpu_req_tag          (fpu_req_tag         ),
    .fpu_rsp_valid        (fpu_rsp_valid       ),
    .fpu_rsp_result       (fpu_rsp_result      ),
`endif
    //exception
...
`ifdef HAS_FPU
// 注: fcc_fj / fpu_rsp_is_fcmp / fcsr_rm / fpu_flags* / fpu_rsp_flags / fcsr_* 等
//     已统一移到 id_stage 实例化之前声明 (见上文), 避免隐式 1-bit 网络截断。
fpu_ctl u_fpu_ctl(
    .clk          (aclk       ),
    .reset        (reset      ),
    .fcc_we       (fcc_we     ),
...
// 流水线 flush 信号（用于清除 CVFPU 中的 stale 操作）
wire pipeline_flush = excp_flush | ertn_flush | refetch_flush | icacop_flush | idle_flush;
