...
module axi2sram_sp_ext #(
    ...
    // 时序优化：引出已寄存的首拍地址（仅供 wrapper 给"写使能 WE 脉冲"判定
    //  BaseRAM/ExtRAM）。addr_o[22] 来自 WRAP 比较长链(CARRY4×7)，是 WE→ODDR
    ...
    output  [AXI_ADDR_WIDTH-1:0]   ax_req_addr_q_o
);
        ...
        case (state_q)
            ...
            WAIT_WVALID: begin
                ...
                if (s_wvalid) begin
                    req_o          = 1'b1;
                    we_o           = 1'b1;
                    // 单拍写优化：首个 beat 写完直接进 WRITE 连续消费，不经 WRITE_NOP
            ...
            READ: begin
                // Pipeline读：输出当前数据的同时发出下一地址请求
                req_o  = 1'b1;
                addr_o = req_addr_q;
                // send the response
                s_rvalid = 1'b1;
                s_rdata  = data_q;  // 寄存后的数据（适配异步SRAM）
                s_rid    = ax_req_q_id;
                s_rlast  = (cnt_q == ax_req_q_len + 1);
                // check that the master is ready
                if (s_rready) begin
                    if (s_rlast) begin
                        // 读burst完成同拍判断：跟SEND_B分支同一套优先级（AR仍优先于AW，
                        // 与IDLE态规则一致），有排队AR就直接转读，没有AR但有AW在等就
                        // 直接转WAIT_WVALID接收，都省去回IDLE再判断的1拍。字段赋值分别
                        // 与下方IDLE态的Read/Write两个分支逐字一致（复用而非重新发明）。
                        if (!ar_fifo_empty) begin
                            ar_fifo_pop    = 1'b1;
                            ax_req_d_id    = ar_fifo_id;
                            ax_req_d_addr  = ar_fifo_addr;
                        ...
                        // Pipeline：计算下一地址并立即发出SRAM请求（省去READ_ADDR的1拍空等）
                        cnt_d = cnt_q + 1;
                        case (ax_req_q_burst)
                            FIXED: begin
                                addr_o = req_addr_q;
                        ...
                        endcase
                        // 保持在READ状态（不经过READ_ADDR）
                        state_d = READ;
                    end
                end
                // backpressure时不推进，保持当前状态
            end
            ...
            // 单拍写优化后正常流水不再进入 WRITE_NOP，保留以防兼容/边界进入
            WRITE_NOP: begin
                s_wready = 1'b0;
                state_d  = WRITE;
            end

            // 单拍写：保持在 WRITE，每拍消费一个 w-beat（去掉原2拍 WRITE_NOP 间隔）
            WRITE: begin
    ...
    assign ax_req_addr_q_o = ax_req_q_addr;
