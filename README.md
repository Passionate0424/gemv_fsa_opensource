...
在该阶段，每一层内部依次要算两类算子——二者都是**访存密集（memory-bound）**、算术强度很低，
瓶颈在把数据搬进算子而不在算力本身：

- **线性层**：Q/K/V/O 投影与 FFN 的 W1/W2/W3（以及最后的 classifier）。由于每步只有单个
  token 向量，这些矩阵乘退化为**矩阵-向量乘（GEMV）**：权重矩阵远大于输入向量，每个权重
  只参与一次乘加就被丢弃，计算强度远低于 GEMM，频繁的权重装载成为主要瓶颈。
- **注意力**：当前 token 的 Q 与 KV cache 中的历史 K/V 做 $QK^\top$ → softmax → $PV$。
  每步同样只有一行 Q，整个 KV cache 读一遍却只做很少的乘加；且 softmax 的逐行全局归约
  使中间结果的访存量随序列长度呈 $O(N^2)$ 增长，规模极易超过片上存储。

**线性层：直接映射为 OS 数据流。** 权重矩阵远大于输入向量、且每层各不相同、算完即弃，
权重驻留毫无意义；故采用**输出驻留（OS）**——部分和在 PE 内原位累加，权重与输入向量
流过阵列，正好契合"输入向量小、权重体量大、层间切换频繁"的访存特征。

**注意力：先在算法层做优化，再据此定硬件数据流。** 这一路分两步：

1. **算法层**先选定 **FlashAttention-2 的在线 softmax**：沿序列维把 K/V 分块，逐块更新
   行最大值 $m$ 与归一化分母 $l$、并对已累加的输出 $O$ 做 rescale，全程不显式存储完整的
   注意力矩阵，从而把访存复杂度由 $O(N^2)$ 降至 $O(N)$。这一步是纯软件层面的算法选择，
   与用什么硬件结构无关。
2. **硬件层**再据该算法的**流式特性**推导数据流。在线 softmax 天然适合流水：注意力分数
   随计算逐行流出、随即做最大值比较与指数变换，无需等整个矩阵就绪。但这要求一条
   "流出—变换—流回"的通路，而本设计的 OS 把部分和封锁在 PE 内、末端才一次性 drain，
   无法在计算中途把中间结果送去比较；**权重驻留（WS）**则天然支持这一需求。
   故注意力模式采用 WS——**Q 驻留在 PE 寄存器，K/V 沿阵列流入**，$QK^\top$ 的累加、
   softmax 的传播与 rescale、$PV$ 的累加在同一条脉动链上流水完成。

两类算子若各做一套阵列，在资源受限的边缘 FPGA 上并不划算。本设计因此让同一套 32-PE
阵列通过可重构分组与上述双模式数据流同时覆盖二者。
