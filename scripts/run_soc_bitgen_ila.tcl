...
#
# 关键策略:
#   综合: Default + retiming（面积紧张，retiming优化PE FMA环路）
...
synth_design -top $top_name -part $part_name \
  -flatten_hierarchy rebuilt -directive Default -retiming \
...
# =============================================================================
# 插入ILA调试核（基于RTL中的mark_debug属性）
# =============================================================================
puts "INFO: Setting up ILA debug core from mark_debug attributes..."

# 获取所有mark_debug网络
set debug_nets [get_nets -hier -filter {MARK_DEBUG == true}]
puts "INFO: Found [llength $debug_nets] mark_debug nets"
...
if {[llength $debug_nets] > 0} {
    ...
    # 连接时钟（使用PLL输出的sys_clk）
    set clk_net [get_nets -hier -filter {NAME =~ *sys_clk_clk_pll}]
    set_property port_width 1 [get_debug_ports u_ila_0/clk]
    connect_debug_port u_ila_0/clk $clk_net

    # 连接第一个probe（使用所有mark_debug信号）
    ...
    puts "INFO: ILA debug core inserted with [llength $debug_nets] signals, depth=4096"
} else {
    puts "WARNING: No mark_debug nets found, skipping ILA insertion"
}
...
# 增加setup时钟不确定度，为PVT波动预留余量（500ps）
