...
二者在同一层内串行出现，但数据复用方式截然不同，因而适合不同的脉动数据流：

- GEMV 的权重矩阵远大于输入向量、且每层各不相同、算完即弃，权重驻留毫无意义，
  故采用**输出驻留（OS）**——部分和在 PE 内原位累加，权重与输入向量流过阵列。
- 注意力的 softmax 夹在 $QK^\top$ 与 $PV$ 两个矩阵乘之间，中间结果必须先流出阵列做
  最大值比较与指数变换、再重新注入，需要一条"流出—变换—流回"的通路，
  故采用**权重驻留（WS）**——Q 驻留在 PE 寄存器，K/V 沿阵列流入，并融合
  **FlashAttention-2 的在线 softmax**（沿序列维分块、逐块更新行最大值与归一化分母，
  不显式存储完整注意力矩阵），把访存复杂度由 $O(N^2)$ 降至 $O(N)$。

若为两类算子各做一套阵列，在资源受限的边缘 FPGA 上并不划算。本设计因此让同一套 32-PE
阵列通过可重构分组与双模式数据流同时覆盖二者。
