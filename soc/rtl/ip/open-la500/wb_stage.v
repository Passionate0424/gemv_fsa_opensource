...
module wb_stage(
    ...
    output [`WS_TO_FPR_BUS_WD -1:0] ws_to_fpr_bus  ,
    //to ds
...
`ifdef HAS_FPU
wire        ws_is_fpu_inst;
wire [31:0] ws_fpu_result;
wire        ws_fpr_we;
wire [ 4:0] ws_fpr_dest;
wire        ws_fld_inst;
wire        ws_fcc_we;
wire [ 1:0] ws_fcmp_cond;
wire [ 4:0] ws_fj;
wire        ws_fcmp_out;
`endif
...
wire [ 4:0] fpr_waddr;
wire [31:0] fpr_wdata;
assign fpr_we_o   = ws_fpr_we & real_valid & ~ws_excp;
assign fpr_waddr  = ws_fpr_dest;
assign fpr_wdata  = ws_fpu_result;
assign ws_to_fpr_bus = {
    fpr_we_o,                 // 37
    fpr_waddr,                // 36:32
    fpr_wdata                 // 31:0
};
...
`ifdef HAS_FPU
// FCC write back moved to async fpu_rsp path
`else
`endif
